...
describe('Email Auth Production - Reveal Public Key and From Address', () => {
  let circuit: any;
  beforeAll(async () => {
    circuit = await wasm_tester(path.join(__dirname, '../src/email_auth_reveal.circom'), option);
  });

  it('should verify and reveal the public key and from address', async () => {
    const emailFilePath = path.join(__dirname, './emails/recovery_outlook_english_pc.eml');

    const emailRaw = readFileSync(emailFilePath, 'utf8');
    const parsedEmail = await relayerUtils.parseEmail(emailRaw);
    const accountCode = '0x2caf991aa705cbc3fcdba1e42353c8245464394706444c57bf221eab44864ab2';
    ...
    const expectedPubKeyHash = await relayerUtils.publicKeyHash(parsedEmail.publicKey);
    expect(BigInt(expectedPubKeyHash)).toEqual(witness[1 + domainFields.length]);

    // check email nullifier
    const expectedEmailNullifier = await relayerUtils.emailNullifier(parsedEmail.signature);
    ...
    for (let idx = 0; idx < maskedCommandFields.length; ++idx) {
      expect(BigInt(maskedCommandFields[idx])).toEqual(witness[1 + domainFields.length + 3 + idx]);
    }
    ...
    for (let idx = 0; idx < 9; ++idx) {
      const expectedValue =
        idx < expectedFromAddrFields.length ? BigInt(expectedFromAddrFields[idx]) : BigInt(0);
